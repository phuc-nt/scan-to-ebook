...
Stage 1 retry tự động 4 lần trên transient (429, 5xx, timeout, empty content, malformed JSON response) — nâng từ 2 để chịu đợt 429 khi chạy 12 worker song song. **Early-abort deterministic**: cùng lớp lỗi (`_error_class`) lặp 2 lần liên tiếp → abort ngay, không đợi hết retry — trang provider trả lỗi y hệt mỗi lần thì retry thêm chỉ tốn thời gian + tiền. Sau retry/abort, trang fail được ghi `DEAD_PLACEHOLDER` .md (size>0) để `collect_pending_pages` các pass sau skip — trước fix này 1 trang chết bị re-OCR ở mọi pass (all-1 → retry → all-2), kéo cả cuốn từ ~30 phút lên 2h22 (thực đo). Non-transient (403, 400, 401) fail luôn lần đầu — không retry vì là config/auth/quota error, retry vô ích.
