...
Vì sao render PDF→page-image thay vì trích text layer? PDF born-digital (ví dụ Calibre, Quartz export) thường có ToUnicode CMap hỏng — pdftotext yield ký tự rác, trong khi ảnh render qua Gemini OCR đọc sạch. Render→OCR là đường duy nhất nhất quán xử lý được cả PDF scan lẫn PDF text-layer-broken. Backend-chain (pdftoppm → magick → sips) mirror pattern image_ops.py, cross-platform, lên lỗi thì báo hướng dẫn cài theo OS.
