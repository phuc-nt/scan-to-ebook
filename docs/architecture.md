...
**Stage 0: Context Pre-Pass** (mới). Trước khi OCR từng trang, pipeline gọi `context_prepass.py` để trích bối cảnh sách từ 15 sample ảnh (7 trang đầu + 4 giữa + 4 cuối, natural-sort). Một multi-image call duy nhất tới OpenRouter để detect title, author, translator, publisher, year, `pages_per_image` (LLM tự phát hiện xem ảnh có 1 hay 2 trang), mục lục, tên riêng + chính tả chuẩn, thuật ngữ, layout, footnote convention, OCR pitfalls, **và `cover_page` (ảnh bìa màu nếu thấy)** — persist vào `work/context.json` (source-of-truth, hand-editable) + `work/context.md` (mirror render, có note "edit .json thay vì file này). Read sample từ `scans/`, write cache vào `work/` — giữ `scans/` nguyên vẹn, clean-room resume = `rm -rf work/`. **Ngoại lệ duy nhất ghi vào `scans/`**: nếu `metadata.json` còn mặc định (title == slug, do `init` tạo TRƯỚC prepass), prepass backfill title/author/year/translator thật (dò được) ngược vào `scans/metadata.json` để TOC/title-page hiện tên sách thật thay vì slug. Guard `title == slug` đảm bảo KHÔNG đè title user tự đặt; ctx không có title → no-op (giữ slug). Prepass **cache-aware**: nếu `work/context.json` đã tồn tại hợp lệ → skip API, cost 0, re-derive block từ JSON. Prepass **abort-on-fail**: API error hay JSON parse fail → emit `context_fail` event + abort pipeline (exit != 0, không tiêu OCR cost). **Prepass skip-when-useless**: pre-pass chỉ phục vụ OCR nên `all` đếm todo TRƯỚC — 0 trang cần OCR (rebuild từ md cache) → bỏ qua pre-pass hẳn (không cost, không dính provider moderation trên ảnh mẫu); `--skip-prepass` ép bỏ qua khi còn trang (dùng cache context nếu có, không thì base prompt). Bare `ocr` cũng auto-load `work/context.json` cạnh output dir (re-derive qua render_block) để pass retry giữ nguyên chất lượng context — `--no-context` tắt, `--context <file>` override. Mẫu ảnh downscale ~1200px (cross-platform: sips macOS → magick ImageMagick → heif-convert → pillow-heif, first available) để tránh HTTP 413; ảnh gốc full-res giữ nguyên cho OCR thực. Spread guidance ("ẢNH TRANG ĐÔI") được render vào block CHỈ khi `pages_per_image >= 2` (conditional per-book, không hardcode base PROMPT). Smoke + full share một prepass (cost counted once, full = cache hit). **Cover/colophon rule**: Block luôn cấm OCR dùng `## `/`### ` heading cho trang bìa/tựa đề (đầu) và trang thông tin xuất bản/colophon (cuối: tên sách, tác giả, NXB, giấy phép, giá bán) — để dạng đoạn thường tránh pandoc `--toc` nhặt chữ trang trí vào mục lục. **Heading-consistency rule**: Block luôn buộc MỌI tiêu đề bài/chương dùng CÙNG `## ` (level-2), sách song ngữ thì CẢ tựa gốc lẫn tựa dịch đều `## ` — vì `--toc-depth=2` chỉ nhặt `## `, tựa lỡ thành `### ` sẽ rớt TOC (bug La Fontaine: bản dịch "Chó rừng và chó giữ nhà" mất khỏi mục lục). **Verse rule** (mới): prepass detect `content_type` ("verse"|"prose"|"mixed"); khi verse/mixed, block buộc OCR kết mỗi câu thơ bằng HAI dấu cách cuối dòng (Markdown hard break → pandoc `<br/>`) để thơ giữ xuống dòng từng câu (không bị Markdown gộp các dòng `\n` liền kề thành một đoạn văn xuôi). Sách văn xuôi không emit rule này. **Known limit**: `post_process` đọc page bằng `read_text().strip()` nên 2-space ở DÒNG CUỐI mỗi trang bị xoá → khổ thơ cắt đúng mép trang có thể mất 1 hard break (hiếm; chấp nhận để giữ scope prompt/pre-pass).

**Stage 1: OCR** (trước gọi là Stage 1). Input là một thư mục PNG/JPG (HEIC/HEIF đã convert→JPG tại import stage), mỗi file là một trang trong `scans/`. Output là một thư mục `.md` trong `work/ocr/`, mỗi trang một file `page_NNN.md`. Module này gọi OpenRouter API qua urllib stdlib (không có httpx/requests dependency), encode ảnh base64, gửi prompt verified cho model OCR mặc định (qwen3.7-plus, override qua `--model`/`OCR_MODEL`) + context block từ Stage 0 append vào. **OCR + context-prepass prompt được chọn theo ngôn ngữ**: registry `PROMPTS` trong `ocr.py` keyed by `lang` từ `metadata.json` (vi → base PROMPT tiếng Việt, ja → `JA_PROMPT` dọc+RTL); registry tương ứng trong `context_prepass.py` chọn context prompt (vi → `CONTEXT_PROMPT` tiếng Việt, ja → `CONTEXT_PROMPT_JA` phát hiện tategaki/spread RTL). Base PROMPT tiếng Việt giữ byte-for-byte unchanged, registry chỉ là selector. Nhận markdown thuần về. ThreadPoolExecutor chạy 12 worker song song (configurable `--workers`) để tận dụng concurrent HTTP. Resumable: trước khi gọi API cho mỗi trang, check xem `work/ocr/page_NNN.md` đã có và non-empty chưa, nếu có thì skip. File `.md` ghi atomic (tmp + `os.replace`) để write bị ngắt giữa chừng không để lại file nửa-ghi đánh lừa resume check. Retry 4 lần với exponential backoff (1s, 2.5s, 5s, 8.5s) trên transient errors (HTTP 429, HTTP 5xx, timeout, empty content, malformed JSON); nhưng nếu CÙNG lớp lỗi lặp lại 2 lần liên tiếp (`_error_class` chuẩn hoá line/col/body-snippet biến thiên) thì **early-abort** — trang deterministic-fail, retry thêm vô ích. Trang trống thật (response rỗng + `finish_reason=stop`) KHÔNG retry mà tự ghi placeholder `<!-- blank page -->`, không tính fail. Trang fail thật (hết retry / early-abort) tự ghi placeholder `<!-- OCR FAILED (deterministic) ... -->` — vẫn tính fail nhưng file size>0 nên pass sau skip, cắt vòng re-OCR cross-pass. CLI hỗ trợ `--dry-run` (đếm trang todo + ước lượng chi phí, không gọi API), `--max-tokens` (default 12000), và env `OCR_MODEL` override model mặc định.

Stage 2 là post-process. Input là thư mục `.md` từ stage 1 (ở `work/ocr/`). Output là một file `book.md` đơn lẻ với YAML front matter, ghi vào `work/`. Module này merge tất cả `page_*.md` theo thứ tự filename (natural-sort số học), strip `` ```markdown `` wrapper nếu model lỡ thêm dù prompt cấm, renumber footnote cross-page (mỗi trang OCR đánh `[^1]` độc lập → shift theo counter chạy để `book.md` không đụng số, tránh pandoc "Duplicate note reference"; bỏ qua `[^N]` trong fenced code block), detect chapter heading bằng regex (CHƯƠNG / Chương / PHẦN / Phần / HỒI / QUYỂN / THIÊN + số La Mã, decimal, hoặc số viết chữ như "thứ nhất", "mười") rồi promote thành h1, và inject YAML metadata (title, author, lang, year) để pandoc dùng. **ATX heading normalization**: Chuẩn hóa không gian sau `#` — model (đặc biệt CJK/space-less fonts) có thể emit `##幽霊の家` mà thiếu space, CommonMark bắt buộc space nên pandoc render thành text thường (không phải heading) → đoạn code tự chèn space: `##幽霊の家` → `## 幽霊の家`. Quy tắc này áp mọi ngôn ngữ (CJK+Việt), critical để chapter headings bị split đúng và vào TOC. Cross-page hyphen-fix intentionally dropped — sẽ giải thích ở phần Design decisions.
...
**Positional slug** accepts both slug (folder name) and path-form arguments (absolute/relative path with `/` or `\`).
...
**Series title auto-derivation**: If `--title` omitted but `--series` AND `--series-index` given, EPUB `dc:title` auto-derived as `"{series} {index:02d}"` (e.g. `Pluto 02`). Explicit `--title` always wins. Series without index → fallback slug. `metadata.json` stores `title: null` when no explicit title (derivation deferred to load time, not frozen at import).
...
Vì sao qwen3.7-plus làm default model? Phase 0 spike ban đầu chọn Gemini 3.1 Pro Preview (so 6 vision model trên 5 trang Nam Phong 1917; zero lỗi chính tả Việt cổ). Lần benchmark sau (modern book + Nam Phong 1917) cho thấy qwen3.7-plus đạt cùng mức zero-fail + giữ chính tả cổ (`nhời`, `nhơn`, hyphenation) nhưng rẻ ~15× (~$0.003/page so với ~$0.05/page của Gemini) → đổi default sang qwen3.7-plus. Pipeline model-agnostic: bất kỳ vision model OpenRouter nào cũng chạy qua `--model`/`OCR_MODEL`. Default ổn định tới khi OpenRouter cập nhật giá hoặc có model rẻ-tốt hơn.
...
Vì sao ThreadPoolExecutor chứ không async? OpenRouter rate limit khoảng 60 req/min cho free tier, 600 req/min cho paid. Với latency 30–60s/page, parallel 12 worker (default) saturate tốt paid tier mà chưa chạm rate-limit (verified 331 trang, 0 fail). Async sẽ phức tạp code mà không cải thiện throughput. Sàn cứng của tổng thời gian là trang chậm nhất: một trang "stutter" (model lặp/lạc khi sinh, nổ output token, latency 200–450s) không rút ngắn được bằng thêm worker, chỉ là nó không còn khoá batch khi nhiều worker chạy. Tăng worker > 12 chỉ khi key chịu được — quá ngưỡng 429 thì lợi suất âm.
...
Stage 1 retry tự động 4 lần trên transient (429, 5xx, timeout, empty content, malformed JSON response) — nâng từ 2 để chịu đợt 429 khi chạy 12 worker song song. **Early-abort deterministic**: CHỈ lỗi content-class (`empty content`/`malformed response`) được đếm — cùng lớp lỗi (`_error_class`) lặp 2 lần liên tiếp → raise `DeadPageError` ngay, không đợi hết retry (provider đọc cùng ảnh trả cùng kết quả hỏng thì retry thêm chỉ tốn thời gian + tiền). Lỗi HẠ TẦNG (429/5xx/timeout) KHÔNG BAO GIỜ đếm vào counter này dù lặp y hệt — burst rate-limit/incident trả lỗi giống nhau trong 1-2s không nói gì về trang; chúng hưởng trọn retry budget, hết vòng raise thường (trang trống, pass sau cứu). **HTTP 400 content-deterministic** (marker `data_inspection_failed` — provider moderation chặn ảnh, hoặc `image format is illegal`) → `DeadPageError` NGAY lần đầu, không retry: cùng ảnh không bao giờ cho kết quả khác; không placeholder hoá thì trang kẹt `todo` vĩnh viễn → mọi pass `all` fail>0 → sách không bao giờ build được (WARN loop vô hạn trong batch). **CHỈ `DeadPageError` mới được ghi `DEAD_PLACEHOLDER`** .md (size>0) để `collect_pending_pages` các pass sau skip — trước fix này 1 trang chết bị re-OCR ở mọi pass (all-1 → retry → all-2), kéo cả cuốn từ ~30 phút lên 2h22 (thực đo). Fail loại khác (402 hết credit, 401/403 config, 400 không-marker, transient hết vòng retry) KHÔNG placeholder — trang để trống cho pass retry / lần chạy lại sau nạp credit OCR tiếp (placeholder hoá chúng = mất nội dung vĩnh viễn mà fail=0 + verify vẫn xanh). Trước khi build, `all` quét `list_dead_pages` và cảnh báo to danh sách trang thiếu nội dung (kèm `dead_pages` trong JSON summary). Non-transient (403, 400, 401) fail luôn lần đầu — không retry vì là config/auth/quota error, retry vô ích.
...
Có `tests/test_page_order_and_retry.py` (pytest) cover các regression: natural-sort page order (sort key, merge order, collect order), retry classification (transient retry / non-transient 4xx fail ngay), early-abort deterministic (cùng error-class abort sau 2 lần; class đổi mỗi lần retry hết; `_error_class` normalization), dead-placeholder (run_batch ghi placeholder cho trang fail → pass sau skip), và sidecar filter. Chạy `python -m pytest`. Test thủ công bằng pilot Nam Phong 1917 (75 trang, có ở `tests/input/Nam Phong Tap Chi Q01_QN_001-006_T001/`). Future test suite nên thêm:
